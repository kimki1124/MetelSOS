			...
			<div id="content">
				...
				<section id="widget-grid" class="">
					<div class="row">
						<article class="col-sm-6">
							<div class="jarviswidget jarviswidget-color-darken" data-widget-editbutton="false" data-widget-deletebutton="false">
								<header>
									...
									<h2>${currYear }년 ${selectMonth }월 우수사원</h2>
								</header>
								<div>
									...
									<div class="widget-body">
										<div class="table-responsive">
											<table class="table table-bordered" id="exclntStfTable">
												<thead>
													<tr>
												...
												<tbody>
													<c:forEach var="item" items="${exclntstfList}" varStatus="status">
														<tr>
															<td>${status.count }</td>
															<td>${item.engineer_dept }</td>
															<td>${item.engineer_name }</td>
															<td>${item.engineer_position }</td>
															<td style="text-align:center;"><a href="javascript:deleteExclntStf('${item.engineer_dept }', '${item.engineer_name }')"><i class="fa fa-ban fa-lg" aria-hidden="true"></i></a></td>
														</tr>
													</c:forEach>
												</tbody>
											</table>
									...
									<div class="widget-body" style="min-height:40px;">
										<button id="showEnrollStfForm" class="btn btn-success">우수사원 등록</button>
						...
						<article class="col-sm-6">
							<div class="jarviswidget jarviswidget-color-darken" id="exclntstfadd" data-widget-editbutton="false" data-widget-deletebutton="false" style="display:none;"><!-- style="display:none;" -->
								<header>
									<span class="widget-icon"> <i class="fa fa-table"></i> </span>
									<h2>우수사원 등록</h2>
								</header>
								<div>
									...
									<div class="widget-body no-padding">
										<form id="exclntstf_add_form" name="exclntstf_add_form" class="smart-form" enctype="multipart/form-data">
											<fieldset>
												<section>
												...
												<section>
													<label class="label">우수사원 소감</label>
											...
											<footer>
												...
												<button id="moveExclntStfList" class="btn btn-danger">취소</button>
											</footer>
										</form>
									</div>
								</div>
		...
		<script type="text/javascript">
		  	...
		  	$("#deptSelect").change(function(){
		  		$( "#deptSelect option:selected" ).each(function() {
		  			var dept = $( this ).text();
		  			$.ajax({
		  				url:'/metelSOS/setEngineerNameList.do',
		  				type:'POST',
		  				dataType:'json',
		  				data:{"engineer_dept" : encodeURI(dept)},
		  	...
		  	$("#showEnrollStfForm").click(function(e){
		  		e.preventDefault();
		  		$("#exclntstfadd").css("display", '');
		  	});
		  	
		  	$("#moveExclntStfList").click(function(e){
		  		e.preventDefault();
		  		$("#exclntstfadd").css('display', 'none');
		  	});
		  	
		  	$("#moveYearMonthList").click(function(e){
		  		e.preventDefault();
		  		var comSubmit = new ComSubmit();
	  			comSubmit.setUrl("/metelSOS/moveManageExclntStfPage.do");
	  			comSubmit.addParam('userType', "engineer");
	  			comSubmit.addParam('menuTitle', encodeURI("우수사원 관리"));
			...
			$.validator.addMethod("valueNotEquals", function(value, element, arg){
		  	...
		  	function deleteExclntStf(dept, name){
		  		$.ajax({
					url:"/metelSOS/deleteExclntStf.do",
					type:'POST',
					dataType:'json',
					data:{ "engineer_dept" : encodeURI(dept), "engineer_name" : encodeURI(name) },
					success:function(msg){
						if(msg.resultMsg == 'SUCCESS'){
							$("#exclntStfTable tr").each(function(){
